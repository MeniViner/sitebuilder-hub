        ...
        <div className="min-h-0 flex-1 overflow-y-auto p-5">
          ...
          <div className="mt-4 grid gap-4 xl:grid-cols-[1.1fr_0.9fr]">
            <section className="soft-panel p-4">
              ...
              <div className="mt-4 rounded-lg border p-3" style={{ background: "var(--surface-muted)", borderColor: "var(--border)" }}>
                <p className="field-label">Target paths</p>
                {targetPaths.length ? (
              ...
              {backupSafety ? (
                <div className="grid gap-3 text-sm">
                  <div className="grid grid-cols-2 gap-2">
                    ...
                    <div>
                      <p className="field-label">Satisfied</p>
          ...
          <div className="mt-4 grid gap-3 md:grid-cols-[1fr_16rem]">
            <label className="block text-sm">
              <span className="field-label">סיבת החלטה</span>
            ...
            <label className="block text-sm">
              <span className="field-label">הקלד {confirmWord} לאישור פעולה</span>
      ...
      {!loading && !error ? (
        <>
          <div className="grid gap-3 sm:grid-cols-2 xl:grid-cols-5">
            <KpiCard title="ממתינים לאישור" value={formatNumber(counts.awaiting)} icon={<CheckCircle2 size={18} />} description="דורשים אישור Admin" tone={counts.awaiting ? "warning" : "neutral"} />
            <KpiCard title="בתור" value={formatNumber(counts.queued)} icon={<Workflow size={18} />} description="ממתינים להרצה" tone="neutral" />
            <KpiCard title="בתהליך" value={formatNumber(counts.active)} icon={<Workflow size={18} />} description="Preflight, ריצה או אימות" tone="info" />
            <KpiCard title="הצליחו" value={formatNumber(counts.succeeded)} icon={<Workflow size={18} />} description="Jobs שהסתיימו בהצלחה" tone="success" />
            <KpiCard title="נכשלו" value={formatNumber(counts.failed)} icon={<Workflow size={18} />} description="דורשים תחקור" tone={counts.failed ? "danger" : "success"} />
          </div>
          ...
          >
            <FilterBar>
              <label className="block">
                ...
                <select className="control" value={statusFilter} onChange={(e) => setStatusFilter(e.target.value)}>
                  ...
                  <option value="awaiting-approval">ממתין לאישור</option>
            ...
            <DataTable columns={["סוג", "סטטוס", "התקדמות", "אישור", "נוצר", "הסתיים", "שגיאה", "פעולות"]} minWidth={1240}>
              {filteredJobs.length === 0 ? (
                <tr><td colSpan={8}><EmptyState title="אין Jobs להצגה" description="שנה סינון או המתן לפעולות חדשות." /></td></tr>
                ...
                <tr key={job._id}>
                  ...
                  <td><span className={`badge ${jobStatusBadgeClass(job.status)}`}>{jobStatusLabel(job.status)}</span></td>
                  <td>
                  ...
                  <td className="max-w-[260px]">
                    {job.status === "awaiting-approval" ? (
                      <div className="space-y-1">
                        <span className="badge badge-warning">דורש אישור</span>
                  ...
                  <td className="num text-xs">{formatDateTime(job.finishedAt)}</td>
                  <td className="max-w-[240px] truncate text-sm" style={{ color: job.errorMessage ? "var(--danger)" : "var(--text-muted)" }} title={job.errorMessage || ""}>{job.errorMessage || "-"}</td>
                  <td>
        ...
        {selectedJob ? (
          <div className="space-y-5">
            <div className="grid gap-3 md:grid-cols-2">
              <div className="soft-panel p-3"><p className="field-label">סטטוס</p><span className={`badge ${jobStatusBadgeClass(selectedJob.status)}`}>{jobStatusLabel(selectedJob.status)}</span></div>
              <div className="soft-panel p-3"><p className="field-label">התקדמות</p><p className="num">{selectedJob.progressPercent || 0}%</p></div>
              <div className="soft-panel p-3"><p className="field-label">נוצר על ידי</p><p>{selectedJob.createdBy || "system"}</p></div>
              <div className="soft-panel p-3"><p className="field-label">נוצר</p><p className="num">{formatDateTime(selectedJob.createdAt)}</p></div>
            </div>
            {selectedJob.errorMessage ? (
              <div className="rounded-lg border p-3 text-sm" style={{ background: "var(--danger-soft)", color: "var(--danger)", borderColor: "var(--border)" }}>{selectedJob.errorMessage}</div>
            ) : null}
            {selectedJob.requiresApproval ? (
              <SectionCard title="Approval" compact>
                <div className="grid gap-3 md:grid-cols-2">
                  <div className="soft-panel p-3">
                    <p className="field-label">נדרש אישור</p>
            ...
            {selectedJob.targetPaths?.length || selectedJob.result || selectedJob.evidence ? (
              <SectionCard title="Execution Evidence" compact>
                <div className="grid gap-3 md:grid-cols-3">
                  ...
                  <div className="soft-panel p-3">
                    <p className="field-label">Evidence rows</p>
                ...
                {selectedJob.targetPaths?.length ? (
                  <div className="mt-3 rounded-lg border p-3" style={{ background: "var(--surface-muted)", borderColor: "var(--border)" }}>
                    <p className="field-label">Target paths sample</p>
            ...
            <SectionCard title="Logs" compact>
